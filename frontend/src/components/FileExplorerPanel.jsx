...
export default function FileExplorerPanel({
  ...
  return (
    <div
      className={`lg:col-span-3 bg-cyber-card border border-cyber-cyan/35 rounded p-4 flex flex-col shadow-neon-cyan h-[450px] lg:h-[550px] ${
    ...
    >
      <div className="border-b border-cyber-cyan/20 pb-2 mb-3">
        ...
        <p className="text-[9px] text-cyber-cyan mt-0.5 uppercase font-bold">
          ROOT &gt; {currentPath.substring(1) || "SYSTEM"}
        </p>
      </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-1.5 pr-1 text-xs">
          {currentPath !== "/" && (
            ...
            >
              ...
              <span className="text-[8px] uppercase text-cyber-cyan/30">
                parent dir
              </span>
            </div>
          )}
          {Object.entries(getContentsAtPath()).map(([name, val]) => {
            ...
            const isHidden =
              name.startsWith(".") ||
              val.hidden ||
              ["author", "creation", "type", "hidden", "password"].includes(
                name,
              );
            const isProtected = val.type === "Protected File";
            ...
            return (
              <div
                key={name}
                onClick={() => {
                  if (!isProtected) handleVisualClick(name, isDir);
                }}
              ...
              >
                ...
                <span className="text-[8px] uppercase">
                  {isReadme ? (
                    <span className="text-cyber-cyan font-bold animate-pulse">
                      Touch to Open
                    </span>
                  ) : isDir ? (
